...
__attribute__((section("ewram_data"))) static u32 sFiller[2] = {};
...
static void ShowDecorationOnMap_(u16 mapX, u16 mapY, u8 decWidth, u8 decHeight, u16 decoration)
{
    ...
    for (j = 0; j < decHeight; j++)
    {
        ...
        {
            x = mapX + i;
            behavior = GetBehaviorByMetatileId(512 + gDecorations[decoration].tiles[j * decWidth + i]);
...
static bool8 CanPlaceDecoration(u8 taskId, const struct Decoration *decoration)
{
    ...
    case DECORPERM_PASS_FLOOR:
        for (i = 0; i < mapY; i++)
        {
            ...
            for (j = 0; j < mapX; j++)
            {
                ...
                behaviorBy = GetBehaviorByMetatileId(512 + decoration->tiles[(mapY - 1 - i) * mapX + j]) & 0xF000;
    ...
    case DECORPERM_BEHIND_FLOOR:
        ...
        for (j = 0; j < mapX; j++)
        {
            curX = gTasks[taskId].data[0] + j;
            behaviorAt = MapGridGetMetatileBehaviorAt(curX, curY);
            behaviorBy = GetBehaviorByMetatileId(512 + decoration->tiles[j]) & 0xF000;
